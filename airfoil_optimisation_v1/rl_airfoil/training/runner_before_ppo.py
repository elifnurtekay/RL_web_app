...
def train_sac(
    ...
    if len(train_steps_df) > 0 and "CL_CD" in train_steps_df.columns:
        ...
        feasible_mask = (
            ...
            & _bool_series(train_steps_df, "is_tc_feasible")
            & _bool_series(train_steps_df, "is_geometry_valid")
            & (pd.to_numeric(train_steps_df.get("local_thickness_violation", 0.0), errors="coerce") <= 0.0)
            & train_steps_df["CL_CD_numeric"].notna()
        )

        feasible_df = train_steps_df[feasible_mask].copy()
    else:
        feasible_df = pd.DataFrame()

    if len(feasible_df) > 0:
        best_train_feasible_row = feasible_df.loc[
            feasible_df["CL_CD_numeric"].idxmax()
        ].to_dict()
    else:
        best_train_feasible_row = None
...
def evaluate_td3(
    ...
    rollout_cols = ["experiment_id","algorithm","evaluator","seed","episode_id","step_id","global_step",
                    ...
                    "min_local_thickness",
                    "area_proxy",
                    "done","truncated","terminated","done_reason","solver_status","solver_error_message"]
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            row = {"experiment_id": run_dir.name, "algorithm": "TD3", "evaluator": cfg.evaluator, "seed": cfg.seed, "episode_id": ep, "step_id": step, "global_step": global_step,
                   ...
                   "CM_lower_violation": info["CM_lower_violation"], "CM_upper_violation": info["CM_upper_violation"], "tc_lower_violation": info["tc_lower_violation"], "tc_upper_violation": info["tc_upper_violation"], "local_thickness_violation": info.get("local_thickness_violation", 0.0), "max_thickness": info.get("max_thickness", ""),
                    ...
                    "min_local_thickness": info.get("min_local_thickness", ""),
                    "area_proxy": info.get("area_proxy", ""),
                   "done": done, "truncated": truncated, "terminated": terminated, "done_reason": info["done_reason"],"solver_status": info.get("solver_status", ""),
                    "solver_error_message": info.get("solver_error_message", ""),
            ...
            cl_cd_value = float(info.get("CL_CD", -np.inf))
            cd_value = float(info.get("CD", np.inf))
            local_violation = float(info.get("local_thickness_violation", 0.0))
            min_local_thickness = float(info.get("min_local_thickness", 0.0))

            is_step_feasible = (
                bool(info.get("is_CM_feasible", False))
                and bool(info.get("is_tc_feasible", False))
                and bool(info.get("is_geometry_valid", False))
                and info.get("solver_status", "ok") in {"ok", "valid", ""}
                and np.isfinite(cl_cd_value)
                and np.isfinite(cd_value)
                and cd_value > 0.0
                and local_violation <= 0.0
                and min_local_thickness >= float(cfg.geometry.min_local_thickness_required)
            )
            ...
            global_step += 1
            last_info = info
        final_is_cm_feasible = bool(last_info.get("is_CM_feasible", False))
        final_is_tc_feasible = bool(last_info.get("is_tc_feasible", False))
        final_is_geometry_valid = bool(last_info.get("is_geometry_valid", False))

        final_is_feasible = (
            final_is_cm_feasible
            and final_is_tc_feasible
            and final_is_geometry_valid
        )
        ...
        summaries.append({
            ...
            "done_reason": last_info["done_reason"],
            "episode_length": step,
        ...
        summaries.append(
            {
                ...
                "solver_error_count": int(
                    str(last_info.get("solver_status", "ok"))
                    not in {"ok", "valid", "", "invalid_geometry", "non_positive_cd_clipped"}
                ),
